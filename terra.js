...
const RPC_ENDPOINT = 'https://terra-rpc.publicnode.com:443/9a353f27b9e92ea909491d7ae2102facbd105fb06ff969932dd19cb31d93d0a6';
...
const DENOM = process.env.DENOM || 'uluna';
...
async function getGranteeWallet() {
  ...
  const wallet = await DirectSecp256k1HdWallet.fromMnemonic(GRANTEE_MNEMONIC, {
    prefix: 'terra',
    hdPaths: [stringToPath("m/44'/330'/0'/0/0")]  // Terra coin type is 330
  });
...
async function getSigningClient() {
  ...
  const client = await SigningStargateClient.connectWithSigner(RPC_ENDPOINT, wallet, {
    registry,
    gasPrice: GasPrice.fromString(`0.34${DENOM}`),
  });
